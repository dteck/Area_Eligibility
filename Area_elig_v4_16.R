...
if(file.exists(paste(workdir,'/Queue/Queue.rds',sep = ""))==TRUE){
  ...
  #added statefp column
  shapeData$STATEFP<-substr(as.character(shapeData$GEOID), 0, 2)
  #------
...
calculations<-function(FIPS, state, Threshold){
  ...
  state_Thresh_no<-subset.data.frame(state_Thresh, state_Thresh$ELIGFY16=="NO")
  #------
...
for (i in seq(1,56,1)){ #separate by FIPS code
  if (i %in% c(3,7,14,43,52)){
    ...
    if(Queue[[i]]$Done[1]==1){
      ...
      BGYes<-subset.data.frame(Output, Output$ELIGFY16=='YES')
      BGNo<-subset.data.frame(Output, Output$ELIGFY16=='NO')
