...
calculations<-function(FIPS, state, Threshold){
  ...
  state_Thresh_no<-subset.data.frame(state_Thresh, state_Thresh$ELIGFY18=="No")
  #------
...
for (i in seq(1,56,1)){ #separate by FIPS code
  if (i %in% c(3,7,14,43,52)){
    ...
    if(Queue[[i]]$Done[1]==1){
      ...
      BGYes<-subset.data.frame(Output, Output$ELIGFY18=='Yes')
      BGNo<-subset.data.frame(Output, Output$ELIGFY18=='No')
